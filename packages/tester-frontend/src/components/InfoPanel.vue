...
const { inputVisualInfo, currentDevice } = storeToRefs(dualsenseStore)
const { t } = useI18n()
...
<template>
    <div v-if="!currentDevice || !inputVisualInfo"
        class="h-full flex items-center justify-center text-lg text-primary/70 select-none">
        {{ $t('info_panel.tips') }}
    </div>
    <div v-else class="flex flex-col items-center max-w-650px mx-auto">
