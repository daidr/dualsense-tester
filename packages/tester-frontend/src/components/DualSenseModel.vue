...
const rightStickPosition = computed(() => {
    return {
        x: getStickPoint(state.value.axes.rightStickX),
        y: getStickPoint(state.value.axes.rightStickY)
    }
})

const leftStickPosition = computed(() => {
    return {
        x: getStickPoint(state.value.axes.leftStickX),
        y: getStickPoint(state.value.axes.leftStickY)
    }
})
</script>
...
<template>
    <svg viewBox="0 0 1117 892">
        ...
        <g id="r3group">
            ...
            <circle v-if="showValue" cx="763.456" cy="528.548" r="2" class="ds-fill-red" :style="{
                transform: `translate(${rightStickPosition.x}px, ${rightStickPosition.y}px)`
            }" />
        </g>
        <g id="l3group">
            ...
            <!-- central point -->
            <circle v-if="showValue" cx="351.764" cy="528.548" r="2" class="ds-fill-red" :style="{
                transform: `translate(${leftStickPosition.x}px, ${leftStickPosition.y}px)`
            }" />
        </g>
        <path id="ps"
