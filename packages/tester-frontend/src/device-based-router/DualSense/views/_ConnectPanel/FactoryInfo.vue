...
import LabeledValue from '@/components/common/LabeledValue.vue';
...
<template>
  <template v-if="hardwareInfo">
    ...
    <div v-if="showFactoryInfo" class="text-primary dou-sc-colorborder rounded-2xl p-1 factory-items">
      <div class="flex flex-col">
        <LocaleLabeledValue v-for="item of hardwareInfo" :key="item.label"
          :label="`connect_panel.factory_info.${item.label}`" :value="item.value"
          :valueLocalePrefix="item.valueLocalePrefix" />
      </div>
    </div>
  </template>
</template>
