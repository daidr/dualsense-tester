...
import { MuteButtonLedControl, PlayerLedBrightness, PlayerLedControl } from '@/device-based-router/_utils/ds.type';
import { bitShiftByte } from '@/utils/format.util';
import { refWithHandler } from '@/utils/reactive.util';
import { computed, onMounted, ref, watch, type Ref } from 'vue';
import { useI18n } from 'vue-i18n';
import { OutputStruct } from './_OutputPanel/outputStruct';
import { sendOutputReportFactory } from '@/device-based-router/_utils/ds.util';
  ...
  <table>
    <tbody>
      ...
      <tr>
        ...
        <td class="value">
          <div>
            <SelfResettingSlider class="w-full" :min="0" :max="255" v-model="motorLeft" />
          </div>
        </td>
      </tr>
      <tr>
        ...
        <td class="value">
          <div>
            <SelfResettingSlider class="w-full" :min="0" :max="255" v-model="motorRight" />
          </div>
        </td>
      </tr>
      <TriggerEffect :struct="struct" is="left" @effect-update="handleTriggerUpdate" />
      <TriggerEffect :struct="struct" is="right" @effect-update="handleTriggerUpdate" />
    </tbody>
  </table>
</template>
